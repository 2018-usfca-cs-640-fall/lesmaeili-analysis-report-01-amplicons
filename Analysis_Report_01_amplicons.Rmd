...
**Figure 1** Abundance of bacteria across age groups. The largest number of 
sequences were seen in age group 25 years old.  

```{r subset-fingers-look-at-taxa}
...
plot_bar(finger_phyloseq_obj,
         ...
         fill = "Order")
```
** Figure 2** Diversity of phyla in age group 25 years old, colored by class.  
...
```
**Figure3** Diversity of phyla in age group 33
```
age_phyloseq_obj <- subset_samples(phyloseq_obj, age =="36")
plot_bar(age_phyloseq_obj, x = "Class")
```
**Figure4** Diversity of phyla in age group 36
```
age_phyloseq_obj <- subset_samples(phyloseq_obj, age =="25")
qplot(x=age, y=Sex,geom="point")
```
